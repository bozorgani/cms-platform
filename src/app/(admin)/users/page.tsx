...
export default function UsersPage() {
  ...
  async function handleDelete(user: User) {
    if (user._id === currentUserId) {
      toast.warning('نمی‌توانید حساب خود را حذف کنید');
      return;
    }
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
                ...
                <tr>
                  <td colSpan={5} className="p-0">
                    <EmptyState
                      ...
                      description={search ? 'جستجوی خود را تغییر دهید.' : 'هنوز کاربری ایجاد نشده.'}
                    />
                  </td>
                </tr>
              ) : (
                users.map((user) => (
                  <tr key={user._id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="p-3 lg:p-4">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${ROLE_COLORS[user.role] || 'bg-gray-100'}`}>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800">
        ...
        <p className="mt-2">یا از API endpoint:</p>
        <pre className="bg-white p-2 rounded mt-2 text-xs overflow-x-auto" dir="ltr">POST /api/v1/users</pre>
      </div>
    </div>
  );
}
