...
async function main() {
  console.log('\n=== پلتفرم CMS - ابزار بازنشانی رمز عبور ===\n');
  ...
  if (args.length === 0) {
    ...
    console.log('استفاده: npm run reset-password -- <email> <password>');
    process.exit(1);
  }
  ...
  if (!user) { log.err('کاربر یافت نشد'); await mongoose.disconnect(); process.exit(1); }

  user.passwordHash = await bcrypt.hash(password, 10);
  await user.save();
  await mongoose.disconnect();
  log.ok('رمز عبور بازنشانی شد!');
  console.log('\nایمیل:    ' + email);
  console.log('رمز عبور: ' + '*'.repeat(password.length));
}
